...
async function realYahooFinanceSearch(keyword) {
    const results = [];
    ...
    for (const term of searchTerms) {
        ...
        try {
            ...
            console.error(`🔴 Yahoo Finance搜尋API呼叫失敗:`, error.message);
            console.log('❌ 網路搜尋失敗，Yahoo Finance可能有問題');
        }
    }
    ...
    try {
        switch (action) {
            case 'search': {
                ...
                return new Response(JSON.stringify(realResults), { headers: { 'Content-Type': 'application/json' } });
            }

            case 'prices': {
                const symbols = url.searchParams.get('symbols');
                if (!symbols) {
                    return new Response(JSON.stringify({ error: '請提供股票代碼' }), { status: 400 });
                }
            ...
            default:
                return new Response(JSON.stringify({ error: '無效的動作' }), { status: 400 });
        }
    } catch (error) {
